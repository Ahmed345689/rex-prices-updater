...
MAX_IDLE_CYCLES    = int(os.environ.get("MAX_IDLE_CYCLES", "5"))      # stop after N empty scrolls
HEADLESS           = os.environ.get("HEADLESS", "1") not in ("0","false","False","")
DEBUG_DIR          = Path(os.environ.get("DEBUG_DIR", "."))

# Fields to strip from raw before saving (not used by the app — reduces file size)
STRIP_RAW_KEYS = {
    "animation", "skillStyleSkills", "traits", "celebration",
    "tags", "bindingXml", "bio",
}
...
_current_cap: "Captured | None" = None  # set in scrape() for signal handler


def _build_result(cap: "Captured") -> dict:
    """Flatten raw player records into the final output format."""
    flat: list[dict] = []
    for rec in cap.all_players:
        ...
        # Strip heavy fields not used by the app
        for k in STRIP_RAW_KEYS:
...
def _save_now(cap: "Captured", label: str = "checkpoint") -> None:
    """Write current players to OUTPUT_FILE immediately."""
    try:
        result = _build_result(cap)
        OUTPUT_FILE.parent.mkdir(parents=True, exist_ok=True)
        OUTPUT_FILE.write_text(json.dumps(result, separators=(",", ":"), ensure_ascii=False))
        print(f"[{label}] {result['totalPlayers']} players saved → {OUTPUT_FILE}")
    except Exception as e:
        print(f"[{label}] save failed: {e}", file=sys.stderr)
...
class Captured:
    ...
    def add_players(self, players: list[dict]) -> int:
        ...
        after = len(self.all_players)
        if added > 0 and (after // 100) > (before // 100):
            _save_now(self, "checkpoint")
        return added
...
GLOBAL_TIMEOUT_S = int(os.environ.get("GLOBAL_TIMEOUT_S", "6600"))  # 20 min max

def _on_signal(signum, frame):
    """Save whatever we have when GitHub cancels the job."""
    print(f"\n[signal] received {signum} — saving current players before exit...")
    if _current_cap is not None and _current_cap.all_players:
        _save_now(_current_cap, "signal-save")
        print(f"[signal] done. Exiting.")
    sys.exit(0)
...
def main() -> int:
    ...
    try:
        ...
        # Still try to save whatever was collected
        if _current_cap is not None and _current_cap.all_players:
            _save_now(_current_cap, "error-save")
        return 1
