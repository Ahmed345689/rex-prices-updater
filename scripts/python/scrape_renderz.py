...
def scrape_with_playwright() -> list[dict]:
    ...
    with sync_playwright() as pw:
        ...
        while True:
            ...
            # استدعاء الـ API من داخل المتصفح بنفس الجلسة — يعدّي Cloudflare
            ...
            }""", {"url": PLAYERS_API, "payload": payload})

            if result.get("error"):
                print(f"[playwright] ❌ خطأ في الصفحة {page_num}: {result['error']}")
                if str(result["error"]) == "403":
                    print("[playwright] 403 — انتظار 15s وإعادة المحاولة ...")
                    time.sleep(15)
